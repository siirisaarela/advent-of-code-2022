...
describe("Advent of code day 4", () => {
  ...
  class DayFourSolver {
   ...
   format(): string[][] {
    const formattedData = this.rawData.map((item: string) => item.split(',').map((item: string) => item.split('-').map((item) => parseInt(item))))
  ...
  describe("Part 1", () => {
   it("Computes if section fully overlaps", async () => {
    const solver = new DayFourSolver(sampleData)
    expect(solver.isFullyOverlappingSection([[2, 4], [6, 8]])).toEqual(false)
    expect(solver.isFullyOverlappingSection([[2, 8], [3, 8]])).toEqual(true)
    expect(solver.isFullyOverlappingSection([[6, 6], [4, 6]])).toEqual(true)
    expect(solver.isFullyOverlappingSection([[64, 75], [1, 76]])).toEqual(true)
   });
  ...
  describe("Part 2", () => {
   it("Computes if section has any overlap", async () => {
    const solver = new DayFourSolver(sampleData)
    expect(solver.isOverlappingSection([[2, 4], [6, 8]])).toEqual(false)
    expect(solver.isOverlappingSection([[6, 6], [7, 8]])).toEqual(false)
    expect(solver.isOverlappingSection([[2, 8], [3, 8]])).toEqual(true)
    expect(solver.isOverlappingSection([[64, 75], [1, 76]])).toEqual(true)
    expect(solver.isOverlappingSection([[5, 7], [7, 9]])).toEqual(true)
   });
